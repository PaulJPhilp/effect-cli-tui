...
describe("Input component", () => {
  ...
  it("should render with default value", () => {
    const { lastFrame } = render(
      <Input defaultValue="admin" message="Username:" onSubmit={vi.fn()} />
    );
    const output = lastFrame();
  ...
  it("should display validation error", () => {
    const validateFn = vi.fn((input: string) =>
      input.trim().length === 0 ? "Name is required" : true
    );
    const onSubmit = vi.fn();
    const { lastFrame, stdin } = render(
      <Input message="Name:" onSubmit={onSubmit} validate={validateFn} />
    );
  ...
  it("should accept valid input", () => {
    ...
    const { stdin } = render(
      <Input message="Email:" onSubmit={onSubmit} validate={validateFn} />
    );
  ...
  it("should call onSubmit when validation passes", () => {
    ...
    const { stdin } = render(
      <Input message="Enter value:" onSubmit={onSubmit} validate={validateFn} />
    );
  ...
  it("should call onSubmit without validation", () => {
    ...
    const { stdin } = render(
      <Input message="Enter value:" onSubmit={onSubmit} />
    );
