...
describe('Password component', () => {
  it('should render password prompt', () => {
    const { lastFrame } = render(<Password message="Enter password:" onSubmit={vi.fn()} />)
    const output = lastFrame()
    expect(output).toContain('Enter password:')
  })

  it('should display masked input field', () => {
    const { lastFrame } = render(<Password message="Password:" onSubmit={vi.fn()} />)
    const output = lastFrame()
  ...
  it('should call onSubmit when password entered', () => {
    ...
    render(<Password message="Enter secret:" onSubmit={onSubmit} />)
    expect(onSubmit).toBeDefined()
  })

  it('should show validation error', () => {
    const onSubmit = vi.fn()
    const validateFn = vi.fn((input) => input.length >= 8 || 'Password too short')
    const { lastFrame, stdin } = render(
      <Password message="Create password:" validate={validateFn} onSubmit={onSubmit} />,
    )
  ...
  it('should accept valid password', () => {
    ...
    const { stdin } = render(
      <Password message="Password:" validate={validateFn} onSubmit={onSubmit} />,
    )
  ...
  it('should call onSubmit without validation', () => {
    ...
    const { stdin } = render(<Password message="Password:" onSubmit={onSubmit} />)
  ...
  it('should handle optional validation', () => {
    const { lastFrame } = render(<Password message="Optional pass:" onSubmit={vi.fn()} />)
    const output = lastFrame()
  ...
  it('should not display actual password characters', () => {
    const { lastFrame } = render(<Password message="Secret:" onSubmit={vi.fn()} />)
    const output = lastFrame()
