...
describe('ProgressBar component', () => {
    it('should render progress bar with percentage', () => {
        const { lastFrame } = render(
            <ProgressBar value={50} max={100} showPercentage />
        )
    ...
    it('should render progress bar with label', () => {
        const { lastFrame } = render(
            <ProgressBar value={75} max={100} label="Downloading" />
        )
        const output = lastFrame()
    ...
    it('should calculate percentage correctly', () => {
        const { lastFrame } = render(
            <ProgressBar value={25} max={50} showPercentage />
        )
        const output = lastFrame()
        // 25/50 = 50%
        expect(output).toContain('50%')
    ...
    it('should hide percentage when showPercentage is false', () => {
        const { lastFrame } = render(
            <ProgressBar value={50} max={100} showPercentage={false} />
        )
        const output = lastFrame()
