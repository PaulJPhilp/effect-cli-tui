...
describe('Confirm component', () => {
  it('should render confirmation prompt', () => {
    const { lastFrame } = render(<Confirm message="Continue?" onSubmit={vi.fn()} />)
    const output = lastFrame()
    expect(output).toContain('Continue?')
  })

  it('should show default indicator for true', () => {
    const { lastFrame } = render(<Confirm message="Proceed?" default={true} onSubmit={vi.fn()} />)
    const output = lastFrame()
    expect(output).toContain('Y/n')
  })

  it('should show default indicator for false', () => {
    const { lastFrame } = render(<Confirm message="Proceed?" default={false} onSubmit={vi.fn()} />)
    const output = lastFrame()
    expect(output).toContain('y/N')
  })
  ...
  it('should call onSubmit with true for y/yes input', () => {
    ...
    const { stdin } = render(<Confirm message="Are you sure?" onSubmit={onSubmit} />)
  ...
  it('should call onSubmit with false for n/no input', () => {
    const onSubmit = vi.fn()
    const { stdin } = render(<Confirm message="Continue?" onSubmit={onSubmit} />)
    stdin.write('n\r')
  ...
  it('should display error for invalid input', () => {
    const onSubmit = vi.fn()
    const { lastFrame } = render(<Confirm message="Confirm?" onSubmit={onSubmit} />)
  ...
  it('should accept case-insensitive input', () => {
    const onSubmit = vi.fn()
    const { stdin } = render(<Confirm message="Confirm?" onSubmit={onSubmit} />)
  ...
  it('should use default when empty input submitted', () => {
    const onSubmit = vi.fn()
    const { stdin } = render(<Confirm message="Proceed?" default={true} onSubmit={onSubmit} />)
    stdin.write('\r')
    // Component uses default value when empty input submitted
    expect(onSubmit).toBeDefined()

    const { stdin: stdin2 } = render(
      <Confirm message="Proceed?" default={false} onSubmit={onSubmit} />,
    )
    stdin2.write('\r')
  ...
  it('should clear error when input changes', () => {
    const onSubmit = vi.fn()
    const { stdin, lastFrame } = render(<Confirm message="Confirm?" onSubmit={onSubmit} />)
