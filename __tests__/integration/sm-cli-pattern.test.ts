...
describe('SM-CLI Integration Patterns', () => {
  describe('Real-world CLI Usage', () => {
    ...
    it('should support success/error messaging patterns', async () => {
      ...
      const errorCalls = consoleErrorSpy.mock.calls.map((call) => call[0] as string)
      const allCalls = [...logCalls, ...errorCalls]

      expect(
        allCalls.some((call) => call.includes('ℹ') && call.includes('Connecting to database...')),
      ).toBe(true)
      expect(
        allCalls.some(
          (call) => call.includes('✓') && call.includes('Database connection established'),
        ),
      ).toBe(true)
      expect(
        allCalls.some((call) => call.includes('ℹ') && call.includes('Processing payment...')),
      ).toBe(true)
      expect(
        allCalls.some(
          (call) =>
            call.includes('✗') && call.includes('Payment processing failed: Insufficient funds'),
        ),
      ).toBe(true)
    ...
    it('should work with @effect/cli command patterns', async () => {
      ...
      const mockCommand = Effect.gen(function* (_) {
        ...
        yield* _(displayLines(['Command: deploy', 'Environment: production', 'Version: 1.2.3']))
    ...
    it('should support custom formatting for advanced CLI output', async () => {
      ...
      expect(calls.some((call) => call.includes('▶️') && call.includes('Starting process...'))).toBe(
        true,
      )
      expect(
        calls.some((call) => call.includes('ℹ') && call.includes('System Status Report')),
      ).toBe(true)

      // JSON output - verify it contains the expected data
      const jsonCall = consoleSpy.mock.calls.find(
        (call) => call[0] && typeof call[0] === 'string' && call[0].includes('responseTime'),
      )
    ...
    it('should handle newline control for inline messages', async () => {
      ...
      expect(
        calls.some(
          (call) => call.includes('ℹ') && call.includes('Processing') && !call.startsWith('\n'),
        ),
      ).toBe(true)
      expect(
        calls.some(
          (call) => call.includes('ℹ') && call.includes('... Done!') && !call.startsWith('\n'),
        ),
      ).toBe(true)
      // Empty message with newline outputs '\nℹ ' (prefix + space)
      expect(
        calls.some((call) => call.includes('ℹ') && call.startsWith('\n') && call.trim() === 'ℹ'),
      ).toBe(true)
      expect(
        calls.some((call) => call.includes('✓') && call.includes('All operations completed')),
      ).toBe(true)
      consoleSpy.mockRestore()
    })
  })
