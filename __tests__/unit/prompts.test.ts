...
describe('Interactive Prompt Functions', () => {
  ...
  describe('promptInput - Text Input', () => {
    ...
    it('should handle validation function returning true', async () => {
      ...
      const validator = (input: string) => input.length > 0 ? true : 'Cannot be empty'
      const result = await Effect.runPromise(promptInput('Q:', undefined, validator))
    ...
    it('should pass validation function to inquirer', async () => {
      ...
      const validator = (input: string) => input.includes('@') ? true : 'Must contain @'
  ...
  describe('promptPassword - Hidden Input', () => {
    ...
    it('should pass validation function', async () => {
      ...
      const validator = (pwd: string) => pwd.length >= 8 ? true : 'Min 8 chars'
    ...
    it('should work without validation', async () => {
      ...
      const result = await Effect.runPromise(promptPassword('Password:', undefined))
      expect(result).toBe('any')
    })

    it('should fail with PromptError on error', async () => {
      (inquirer.password as any).mockRejectedValue(new Error('Password prompt failed'))
      const result = await Effect.runPromise(
  ...
  describe('Composition Patterns', () => {
    it('should support chaining multiple prompts', async () => {
      ...
      let callIdx = 0
      ;(inquirer.input as any).mockImplementation(async () => {
    ...
    it('should support mixing different prompt types', async () => {
      ...
      const result = await Effect.runPromise(effect)
      expect(result).toEqual({ name: 'John', confirmed: true, language: 'Python' })
    })
    ...
    it('should support conditional prompting', async () => {
      ...
      let inputIdx = 0
      ;(inquirer.input as any).mockImplementation(async () => {
        return inputResponses[inputIdx++]
      })
      ;(inquirer.confirm as any).mockResolvedValue(true)
  ...
  describe('Validation Edge Cases', () => {
    it('should handle validator returning true for valid input', async () => {
      ...
      const validator = (email: string) => email.includes('@') ? true : 'Invalid'
      const result = await Effect.runPromise(promptInput('Email:', undefined, validator))
    ...
    it('should handle validator returning error message string', async () => {
      ...
      const validator = (email: string) => email.includes('@') ? true : 'Must contain @'
      await Effect.runPromise(promptInput('Email:', undefined, validator))
    ...
    it('should handle complex validation logic', async () => {
      ...
      const validator = (pwd: string) => {
        ...
        return (hasNumber && hasUpperCase && isLongEnough) ? true : 'Invalid password'
      }
