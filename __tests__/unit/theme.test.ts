  ...
  describe('setTheme', () => {
    it('should set and retrieve a custom theme', async () => {
      ...
      const program = Effect.gen(function* () {
        ...
        const retrieved = yield* theme.getTheme()
  ...
  describe('withTheme', () => {
    it('should apply theme temporarily and restore after', async () => {
      const program = Effect.gen(function* () {
        ...
        const initialTheme = yield* theme.getTheme()

        // Use emoji theme temporarily
        const result = yield* theme.withTheme(
          themes.emoji,
          Effect.gen(function* () {
            const tempTheme = yield* theme.getTheme()
        ...
        const restoredTheme = yield* theme.getTheme()
...
describe('Theme Integration with Display Functions', () => {
  it('should use theme icons in display functions', async () => {
    ...
    const combinedOutput = [...logCalls, ...errorCalls]
      .map((call) => call.map((arg) => String(arg ?? '')).join(' '))
      .join('')

    expect(combinedOutput).toContain('✅')
    expect(combinedOutput).toContain('❌')
    expect(combinedOutput).toContain('⚠️')
    expect(combinedOutput).toContain('ℹ️')
