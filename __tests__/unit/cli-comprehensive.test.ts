...
describe('EffectCLI - Comprehensive Coverage', () => {
  ...
  describe('run() - Options', () => {
    ...
    it('should combine cwd and env options', async () => {
      ...
      const program = Effect.gen(function* () {
        const cli = yield* EffectCLI
        const result = yield* cli.run('cmd', [], { cwd: '/home', env: { VAR: 'val' } })
        return result
      }).pipe(Effect.provide(EffectCLI.Default))
  ...
  describe('run() - Error Handling', () => {
    ...
    it('should handle maxBuffer = 0 (unlimited)', async () => {
      ...
      setImmediate(() => {
        mockChild.stdout.emit('data', 'a'.repeat(1000000)) // Very large output
        mockChild.stderr.emit('data', '')
        mockChild.emit('close', 0)
      })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle large combined stdout and stderr', async () => {
      ...
      const program = Effect.gen(function* () {
        const cli = yield* EffectCLI
        const result = yield* cli.run('command', [], { maxBuffer: 1024 * 1024 })
        return result
      }).pipe(Effect.provide(EffectCLI.Default))

      setImmediate(() => {
        mockChild.stdout.emit('data', 'x'.repeat(500000))
        mockChild.stderr.emit('data', 'y'.repeat(500000))
        mockChild.emit('close', 0)
      })

      const result = await Effect.runPromise(program)
      expect(result.stdout).toBe('x'.repeat(500000))
      expect(result.stderr).toBe('y'.repeat(500000))
    })
