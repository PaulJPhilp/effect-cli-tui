  ...
  describe("Welcome Screen", () => {
    it("should display welcome panel with application overview", async () => {
      ...
      await Effect.runPromise(
        displayPanel(welcomeContent, "Welcome to Prompt Builder", {
          type: "info",
        })
      );
    ...
    describe("validateField", () => {
      ...
      it("should validate boolean field", async () => {
        ...
        expect(invalidResult).toContain(
          "must be answered with yes (y) or no (n)"
        );
      });
    });
  ...
  describe("Integration Workflow", () => {
    it("should complete full workflow with valid inputs", async () => {
      ...
      const workflow = Effect.gen(function* () {
        // 1. Welcome screen
        yield* displayPanel("Test content", "Welcome to Prompt Builder", {
          type: "info",
        });
        ...
        yield* validateGeneratedPrompt(promptText).pipe(
          Effect.catchAll(() =>
            Effect.fail(new Error("Prompt validation failed"))
          )
        );
