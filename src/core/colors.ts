...
export function displayHighlight(message: string): Effect.Effect<void> {
  const highlightColor = getCurrentThemeSync().colors.highlight ?? COLOR_HIGHLIGHT
...
export function displayInfo(message: string): Effect.Effect<void> {
  ...
  const styledMessage = applyChalkStyle(message, { color: infoColor })
  return display(styledMessage, { type: DEFAULT_DISPLAY_TYPE })
}
...
export function displayListItem(
  ...
  return Effect.gen(function* () {
    ...
    const bulletColor = options?.color || getCurrentThemeSync().colors.highlight || COLOR_HIGHLIGHT
