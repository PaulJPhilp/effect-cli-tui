...
const REDACTION_PATTERNS = [
  ...
  {
    pattern:
      /github[_-]?token\s*[=:]\s*(?!ghp_|ghu_|ghs_|ghr_|\[REDACTED:)[^\s,;]+/gi,
    name: "GitHub Token",
  },
  ...
  {
    pattern: /(?<![a-z_])secret\s*[=:]\s*(?!\[REDACTED:)[^\s,;]+/gi,
    name: "Secret",
  },
  {
    pattern: /(?<![a-z_])token\s*[=:]\s*(?!\[REDACTED:)[^\s,;]+/gi,
    name: "Token",
  },
];
...
export function redactArguments(command: string, args: string[]): string[] {
  ...
  const commandSensitiveFlags =
    commandSpecificFlags[commandName] || nextArgSensitiveFlags;
  ...
  for (let i = 0; i < redacted.length; i++) {
    ...
    if (
      arg &&
      commandSensitiveFlags.some(
        (f) => f.toLowerCase() === arg.toLowerCase()
      ) &&
      i + 1 < redacted.length
    ) {
