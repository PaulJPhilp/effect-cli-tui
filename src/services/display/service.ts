 ...
 * ```
 */
export class DisplayService extends Effect.Service<DisplayService>()('app/DisplayService', {
  scoped: Effect.sync(
    (): DisplayServiceApi =>
      ({
        output: (message: string, options: DisplayOptions = {}): Effect.Effect<void> => {
          ...
          return type === 'error' ? Console.error(output) : Console.log(output)
        },
        ...
        json: (data: unknown, options: JsonDisplayOptions = {}): Effect.Effect<void> => {
          ...
          return Effect.gen(function* () {
            ...
            const styledPrefix =
              options.style && prefix ? applyChalkStyle(prefix, options.style) : prefix
