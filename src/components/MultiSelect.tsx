...
export const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  useInput(
    (input, key) => {
      ...
      if (key.upArrow) {
        setHighlightedIndex((prev) => (prev > 0 ? prev - 1 : normalizedChoices.length - 1))
        return // Prevent any other processing
      }

      if (key.downArrow) {
        setHighlightedIndex((prev) => (prev < normalizedChoices.length - 1 ? prev + 1 : 0))
        return // Prevent any other processing
      }
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        <Text color="gray"> (Space to toggle, Enter to submit) Selected: {selected.size}</Text>
      </Box>
    </Box>
  )
}
