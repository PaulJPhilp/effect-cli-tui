...
export function displayBox(content: string, options?: BoxStyle): Effect.Effect<void> {
  return Effect.sync(() => {
    ...
    const maxWidth = Math.max(...lines.map((l) => l.length), title?.length || 0) + padding * 2 + 2
    ...
    if (title) {
      ...
      const borderRightLength = maxWidth - borderLeft.length - titleSpace.length - 1
      const borderRight = style.horizontal.repeat(Math.max(0, borderRightLength)) + style.topRight
    ...
    for (let i = 0; i < padding; i++) {
      box += chalk.dim(`${style.vertical} ${' '.repeat(maxWidth - 2)} ${style.vertical}\n`)
